...
void RodState::set_dofs(VecXd const& dofs, bool const init_frames)
{
    m_dirty = true;
...
void RodState::init_reference_frames()
{
...
void RodState::transport_reference_frames(VecXd const& future_dofs)
{
    for (size_t i = 0; i < m_reference_frames.size(); ++i)
    {
        ...
        m_reference_frames[i] = parallel_transport_matrix(tangent_0, tangent_1)
                * m_reference_frames[i]; // NB col(0) should coincide with tangent_1.
    }
}

void RodState::compute_material_frames()
{
